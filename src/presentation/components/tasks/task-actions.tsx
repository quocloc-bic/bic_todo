...
import { useTaskActions } from '@bic_todo/adapters/task-actions';
...
const TaskActions = (props: TaskActionsProps) => {
  const adapter = useTaskActions();

  const initialTask: ITask = {
    id: props.task?.id ?? 0,
    categoryId: props.task?.categoryId ?? adapter.firstCategory?.id ?? 0,
  ...
  const selectedCategory = adapter.categories?.find(
  ...
  const onCreateTask = () => {
    ...
    if (newTask.id > 0) {
      completionPromise = adapter.updateTask(newTask);
    } else {
      completionPromise = adapter.createTask({
        ...newTask,
      });
    }
              ...
              return (
                <Pressable
                  ...
                  <Box
                    ...
                    borderTopEndRadius={
                      adapter.categories?.length - 1 === index
                        ? 'rounded32'
                        : 'none'
                    }
